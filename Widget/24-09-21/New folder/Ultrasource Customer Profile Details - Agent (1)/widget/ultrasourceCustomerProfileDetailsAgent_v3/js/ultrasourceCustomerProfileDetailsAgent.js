...
define(
  ...
  ['knockout', 'notifier','pubsub', 'spinner', 'xDomainProxy','jquery',
   ...
   function (ko, notifier, pubsub, spinner, XDomainProxy,$,
    ...
    // ks added on 8/31/21
    dyProp_nameTitle: ko.observable(),
    dyProp_role: ko.observable(),
    dyProp_positionTitle: ko.observable(),
    dyProp_officeNumber: ko.observable(),
    dyProp_homeNumber: ko.observable(),
    dyProp_mobileNumber: ko.observable(),
    dyProp_faxNumber: ko.observable(),
    // ks added on 9/8/21
    receiveMails: ko.observable(0),
    receiveEmail: ko.observable(0),
    receiveTexts: ko.observable(0),
    receiveCatalogs: ko.observable(0),
    // dyProp_syteLineCustomerID: ko.observable(),
    // dyProp_suppliesSpecialist: ko.observable(),
    // dyProp_receiveCatalog: ko.observable(),
    // dyProp_syteLineContactID: ko.observable(),
    // dyProp_shippingAddress: ko.observable(),
    ...
    beforeAppear: function (page) {
       ...
       //   ks added on 8/30/21
      for(var i = 0; i<user.dynamicProperties().length;i++){
         if(user.dynamicProperties()[i].id()=="name_title"){
             widget.dyProp_nameTitle(user.dynamicProperties()[i].value());
             console.log("lengthhh",widget.dyProp_nameTitle());
         }else if(user.dynamicProperties()[i].id()=="org_role"){
             widget.dyProp_role(user.dynamicProperties()[i].value());
             console.log("roleDisplay",widget.dyProp_role()); 
         }else if(user.dynamicProperties()[i].id()=="position_title"){
             widget.dyProp_positionTitle(user.dynamicProperties()[i].value());
             console.log("ptDisplay",widget.dyProp_positionTitle());
         }else if(user.dynamicProperties()[i].id()=="office_number"){
             widget.dyProp_officeNumber(user.dynamicProperties()[i].value());
             console.log("onDisplay",widget.dyProp_officeNumber());
         }else if(user.dynamicProperties()[i].id()=="home_phone_number"){
             widget.dyProp_homeNumber(user.dynamicProperties()[i].value());
             console.log("onDisplay",widget.dyProp_homeNumber());
         }
         else if(user.dynamicProperties()[i].id()=="mobile_number"){
             widget.dyProp_mobileNumber(user.dynamicProperties()[i].value());
             console.log("mnDisplay",widget.dyProp_mobileNumber());
         }else if(user.dynamicProperties()[i].id()=="fax_number"){
             widget.dyProp_faxNumber(user.dynamicProperties()[i].value());
             console.log("fnDisplay",widget.dyProp_faxNumber());
         }
         else if(user.dynamicProperties()[i].id()=="receive_texts"){
             widget.receiveTexts(user.dynamicProperties()[i].value());
             console.log("receive_texts",widget.receiveTexts());
         }
         else if(user.dynamicProperties()[i].id()=="receive_catalog"){
             widget.receiveCatalogs(user.dynamicProperties()[i].value());
             console.log("receiveCatalogs",widget.receiveCatalogs());
         }
         else if(user.dynamicProperties()[i].id()=="receive_mailings_special_offers"){
             widget.receiveMails(user.dynamicProperties()[i].value());
             console.log("receiveMails",widget.receiveMails());
         }
         
      }
      
      if(user.receiveEmail() == "yes"){
          widget.receiveEmail(true);
      }else{
          widget.receiveEmail(false);
      }
    ...
    //receiveEmail: ko.observable(0)
      /*widget.dyProp_role.subscribe(function (newValue) {
         console.log("selected dyProp_role "+newValue);
          widget.dyProp_role(newValue);
        });
      */
   ...
   /**
    ...
    updateCustomer : function(){
      ...
      for(var i = 0; i<user.dynamicProperties().length;i++){
         if(user.dynamicProperties()[i].id()=="name_title"){
             user.dynamicProperties()[i].value(widget.dyProp_nameTitle());
             
         }else if(user.dynamicProperties()[i].id()=="org_role"){
             user.dynamicProperties()[i].value(widget.dyProp_role());
             
         }else if(user.dynamicProperties()[i].id()=="position_title"){
             user.dynamicProperties()[i].value(widget.dyProp_positionTitle());
             
         }else if(user.dynamicProperties()[i].id()=="office_number"){
             user.dynamicProperties()[i].value(widget.dyProp_officeNumber());
             
         }else if(user.dynamicProperties()[i].id()=="home_phone_number"){
             user.dynamicProperties()[i].value(widget.dyProp_homeNumber());
             
         }
         else if(user.dynamicProperties()[i].id()=="mobile_number"){
             user.dynamicProperties()[i].value(widget.dyProp_mobileNumber());
             
         }else if(user.dynamicProperties()[i].id()=="fax_number"){
             user.dynamicProperties()[i].value(widget.dyProp_faxNumber());
            
         }
         else if(user.dynamicProperties()[i].id()=="receive_texts"){
             user.dynamicProperties()[i].value(widget.receiveTexts());
             
         }
         else if(user.dynamicProperties()[i].id()=="receive_catalog"){
             user.dynamicProperties()[i].value(widget.receiveCatalogs());
             
         }
         else if(user.dynamicProperties()[i].id()=="receive_mailings_special_offers"){
             user.dynamicProperties()[i].value(widget.receiveMails());
             
         }
      }
      
      if( widget.receiveEmail() === true){
          user.receiveEmail("yes");
      }else{
          user.receiveEmail("no");
      }
