...
var getWidget;
    return {
       ...
       */
       clickRequestDetails:function(){
             navigation.goTo('/ContactRequests');
      ...
      clickContactDetails: function(data, index) {
        var widget = this;
         if(widget.status()) {
              $(".RoleNames" ).css("color", "black");
            $(".RoleNamesbox" ).css("outline", "2px solid black");
            } else {
                $(".RoleNames" ).css("color", "gray");
                $(".RoleNamesbox" ).css("outline", "2px solid gray");
            }
...
/*
          if(data[0].receive_texts===""){
          widget.receiveTexts(false);    
          }else{
              widget.receiveTexts(data[0].receive_texts);
          }
         if(data[0].receive_mailings_special_offers===""){
              widget.receiveMails(false);
          }else{
          widget.receiveMails(data[0].receive_mailings_special_offers);
          }
          
           
          
          widget.officeNumber(data[0].office_number);
          widget.mobileNumber(data[0].mobile_number)
          widget.selectedTitle(data[0].name_title);
          widget.faxNumber(data[0].fax_number);
          widget.title(data[0].position_title);
          widget.roleSelected(data[0].org_role);
  */        
 ...
 for(var i=0;i<widget.user().dynamicProperties().length;i++){
         ...
         if(widget.user().dynamicProperties()[i].id()==="receive_texts"){



                         if(widget.receiveTexts()===""){
                    widget.receiveTexts(false);
                }else{
                     if(typeof(widget.user().dynamicProperties()[i].value)==="boolean"){
                    widget.receiveTexts(widget.user().dynamicProperties()[i].value);
                     }else{
                    widget.receiveTexts(widget.user().dynamicProperties()[i].value());
                     }
                }
            ...
            }else if(widget.user().dynamicProperties()[i].id()==="receive_mailings_special_offers"){
                
                                 if(widget.receiveMails()===""){
                    widget.receiveMails(false);
                }else{
                if(typeof(widget.user().dynamicProperties()[i].value)==="boolean"){
                    widget.receiveMails(widget.user().dynamicProperties()[i].value);
                }else{
                    widget.receiveMails(widget.user().dynamicProperties()[i].value());
                }
                }        
            ...
            }else if(widget.user().dynamicProperties()[i].id()==="office_number"){

     if(typeof(widget.user().dynamicProperties()[i].value)==="string"){
                widget.officeNumber(widget.user().dynamicProperties()[i].value);
     }else{
                 widget.officeNumber(widget.user().dynamicProperties()[i].value());
     }       
                
            }else if(widget.user().dynamicProperties()[i].id()==="mobile_number"){
                     if(typeof(widget.user().dynamicProperties()[i].value)==="string"){
                widget.mobileNumber(widget.user().dynamicProperties()[i].value);
}else{
                    widget.mobileNumber(widget.user().dynamicProperties()[i].value());
}
            }else if(widget.user().dynamicProperties()[i].id()==="name_title"){
     if(typeof(widget.user().dynamicProperties()[i].value)==="string"){
        widget.selectedTitle(widget.user().dynamicProperties()[i].value);
     }else{
         widget.selectedTitle(widget.user().dynamicProperties()[i].value());
          }
            }else if(widget.user().dynamicProperties()[i].id()==="fax_number"){
               if(typeof(widget.user().dynamicProperties()[i].value)==="string"){      
                 widget.faxNumber(widget.user().dynamicProperties()[i].value);
               }else{
                   widget.faxNumber(widget.user().dynamicProperties()[i].value());
               }
                
            }
            else if(widget.user().dynamicProperties()[i].id()==="position_title"){
               if(typeof(widget.user().dynamicProperties()[i].value)==="string"){      
                widget.title(widget.user().dynamicProperties()[i].value);
               }else{
                   widget.title(widget.user().dynamicProperties()[i].value());
               }
            }
            else if(widget.user().dynamicProperties()[i].id()==="org_role"){
           if(typeof(widget.user().dynamicProperties()[i].value)==="string"){      
               widget.roleSelected(widget.user().dynamicProperties()[i].value);
           }else{
               widget.roleSelected(widget.user().dynamicProperties()[i].value());
           }
        }
          ...
          this.isStatusModified = true;
                    ...
                    getWidget.roles(true);
          getWidget.selectedTitle("")
          getWidget.firstName('')
          getWidget.lastName('')
          getWidget.officeNumber("")
          getWidget.mobileNumber("")
          getWidget.faxNumber("")
          getWidget.emailAddress("")
          getWidget.roleSelected("")
          getWidget.title("")
          getWidget.receiveMails(false);
          getWidget.receiveTexts(false);
      ...
      createNewContact: function(data, event){
          getWidget.newContact(true);
          getWidget.roles(true);
          getWidget.selectedTitle("")
          getWidget.firstName('')
          getWidget.lastName('')
        ...
        getWidget.isDelegatedAdminFormEdited = false;
        getWidget.contactDetailsMode(true);
        $("#cc-contact-first-name").focus();
        return false;
      },
...
//          notifier.sendError(this.WIDGET_ID, this.translate('ErrorMsg'), true);
 ...
 $('#SaveErrorContactModal').modal('show');
      ...
      koToJS: function() {
        ...
        /*for(var i=0;i<widget.user().dynamicProperties().length;i++){
            ...
            if(widget.user().dynamicProperties()[i].id()==="receive_texts"){
                ...
                if(widget.receiveTexts()===""){
                    widget.user().dynamicProperties()[i].value=false;
                }else{
                widget.user().dynamicProperties()[i].value=widget.receiveTexts();                    
                }
                ...
                widget.user().dynamicProperties()[i].value=widget.receiveMails()
                }
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="office_number"){
                widget.user().dynamicProperties()[i].value=widget.officeNumber();
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="mobile_number"){
                widget.user().dynamicProperties()[i].value=widget.mobileNumber();
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="name_title"){
                widget.user().dynamicProperties()[i].value=widget.selectedTitle();
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="fax_number"){
                widget.user().dynamicProperties()[i].value=widget.faxNumber();
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="position_title"){
                widget.user().dynamicProperties()[i].value=widget.title();
                 this.isDelegatedAdminFormEdited = true ;
            }else if(widget.user().dynamicProperties()[i].id()==="org_role"){
                widget.user().dynamicProperties()[i].value=widget.roleSelected();
                 this.isDelegatedAdminFormEdited = true ;
            }*/
          ...
          //data["dynamicProperties"] = widget.user().dynamicProperties();
        data["office_number"]=widget.officeNumber();
 ...
 validateEmailAddress: function (event) {
		    var label;
				if (event.target.value === "" || event.target.value === null) {
				    ...
				    var flag=getWidget.validateEmail(event.target.value);
					if (flag) {     //correct mobile number
						event.target.style = "border: 2px solid #d3d3d";
				        if(event.target.id !== null){
				            label = event.target.id + "-label"; 
    				        document.getElementById(label).style = "color:black";				        
       ...
       */
       changeState: function(data){
           ...
           console.log("data is   RoleNames"+data)
